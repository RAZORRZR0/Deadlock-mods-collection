...
const buildPath = path.join(repositoryDir, 'build_showrank_barebones_no_missing.ps1');
...
const composedSources = composition.composeBarebonesSources(repositoryDir, packageDir);
...
assert.match(build, /\$noMissingRoot\s*=\s*Join-Path \$root 'showrank_barebones_no_missing'/, 'the source root is the no-missing barebones package');
assert.match(build, /\$vpkOutput\s*=\s*Join-Path \$root 'showrank_barebones_no_missing_dir\.vpk'/, 'the artifact has its dedicated name');
...
assert.doesNotMatch(build, /ShowRankBarebonesMissing|MISSING_|ENEMY MISSING|HealthVisible|GameTime/, 'the no-missing build has no alert-only runtime or clock contract');
...
assert.match(build, /Assert-NoMissingBarebonesAssetSet -Actual \(Get-NoMissingBarebonesAssetPaths -RootPath \$noMissingRoot\) -ExpectedAssets \$requiredSourceAssets -Label 'NoMissingBarebones source package'/, 'the full source inventory is rejected unless exact');
assert.match(build, /Assert-NoMissingBarebonesAssetSet -Actual \(Get-NoMissingBarebonesAssetPaths -RootPath \$noMissingStageSource\) -ExpectedAssets \$requiredSourceAssets -Label 'Composed no-missing-barebones source'/, 'composed staging is rejected unless exact');
...
assert.match(build, /Get-NoMissingBarebonesPackedAssetPaths/, 'packed VPK trees are normalized before validation');
assert.match(build, /function Invoke-NoMissingBarebonesClosureMinification/, 'the staged runtime is minified through a dedicated Closure step');
...
assert.match(runtimeTest, /composition\.composeBarebonesSources\(repositoryDir, packageDir\)\.runtime/, 'readable runtime behavior uses the no-missing host composition');
...
assert.match(profileRuntimeTest, /runtimeAdapter\.source = composition\.composeBarebonesSources\(repositoryDir, packageDir\)\.runtime/, 'the readable profile adapter uses the no-missing host composition');
...
const validateIndex = indexOfRequired('& npm --prefix $noMissingRoot run validate');
const stagedCopyIndex = indexOfRequired('Copy-Item -LiteralPath $sourcePath -Destination $stagedPath -Force');
const compositionIndex = indexOfRequired('& node $compositionScript --host-root $noMissingRoot $noMissingStageSource');
...
const minifyCallIndex = indexOfRequired('Invoke-NoMissingBarebonesClosureMinification -ReadableSourcePath $readableRuntime -StagedSourcePath $stagedRuntime');
const runtimeSmokeIndex = indexOfRequired("& node (Join-Path $noMissingRoot 'tests\\showrank-barebones-runtime.test.js')");
const profileRuntimeSmokeIndex = indexOfRequired("& node (Join-Path $noMissingRoot 'tests\\profile-stats-community-runtime.test.js')");
const compilerIndex = indexOfRequired('Invoke-Source2Compiler -CompilerPath $compiler -SourceDir $noMissingStageSource');
const packIndex = indexOfRequired('Invoke-VpkPack -VpkEditCli $vpkEditCli -InputDir $noMissingStageCompiled');
...
assert.match(build, /Invoke-Source2Compiler[\s\S]*?Assert-NoMissingBarebonesAssetSet -Actual \(Get-NoMissingBarebonesAssetPaths -RootPath \$noMissingStageCompiled\) -ExpectedAssets \$requiredCompiledAssets -Label 'Compiled no-missing-barebones output'/, 'compiler output is strictly checked');
assert.match(build, /Invoke-VpkPack[\s\S]*?Assert-NoMissingBarebonesAssetSet -Actual \(Get-NoMissingBarebonesPackedAssetPaths -Tree \$packedTree\) -ExpectedAssets \$requiredCompiledAssets -Label 'Packed no-missing-barebones VPK'/, 'packed artifact is strictly checked');
...
assert.match(build, /\$temporary\s*=\s*Join-Path \$AddonsPath 'pak89_dir\.showrank-barebones-no-missing\.tmp\.vpk'/, 'installation uses a named temporary artifact');
assert.match(build, /\$replaceBackup\s*=\s*Join-Path \$AddonsPath 'pak89_dir\.showrank-barebones-no-missing\.replace-backup\.tmp\.vpk'/, 'replacement uses a named recoverable backup');
...
assert.match(build, /Assert-NoMissingBarebonesAssetSet -Actual \(Get-NoMissingBarebonesPackedAssetPaths -Tree \$temporaryTree\) -ExpectedAssets \$requiredCompiledAssets -Label 'Temporary no-missing-barebones VPK'/, 'temporary installation tree is strictly checked');
...
assert.match(build, /function Remove-NoMissingBarebonesInstallTemporary[\s\S]*?Assert-PathUnderRoot -Path \$Path -RootPath \$AddonsRoot[\s\S]*?pak89_dir\.showrank-barebones-no-missing\.tmp\.vpk/s, 'temporary cleanup is path-guarded');
assert.match(build, /catch \{\s*Remove-NoMissingBarebonesInstallTemporary -Path \$temporary -AddonsRoot \$AddonsPath\s*throw/s, 'failed installation removes only its guarded temporary artifact and preserves the replacement backup');
assert.match(build, /if \(-not \$KeepStaging\) \{\s*Remove-TreeUnderRoot -Path \$stagingRoot -RootPath \$root -ExpectedLeaf '_showrank_barebones_no_missing_build'/s, '-KeepStaging controls guarded staging cleanup');
...
console.log('showrank barebones no-missing build contract tests passed');
