...
const editableSourcePath = path.join(packageDir, 'panorama', 'scripts', 'showrank_barebones.js');
const runtimePath = process.env.SHOWRANK_BAREBONES_RUNTIME;
const source = runtimePath
  ? fs.readFileSync(runtimePath, 'utf8')
  : composition.composeBarebonesSources(repositoryDir, packageDir).runtime;
...
assert.doesNotMatch(readableSource, /MISSING_|ShowRankBarebonesMissing|ENEMY MISSING|HealthVisible|GameTime/, 'rank-only runtime contains no missing-lane subsystem or clock polling');
assert.doesNotMatch(readableSource, /Announcement|GenericAnnouncement/, 'rank-only runtime contains no announcement subsystem');
