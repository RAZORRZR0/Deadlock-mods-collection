...
World receivers now skip unescaped strings lacking the scan-gate marker before JSON parsing. Escaped strings still parse, and marker matches still require the complete existing gate validation. HUD parsing is unchanged. Compare `worldMessagesSkipped`/`worldCharsSkipped` with `parsedMessages`/`parsedChars` in new captures; `worldMessages` counts only parsed world messages. `scripts/validate-message-filter.js` exercises the pure predicate without Panorama mocks.
...
- Healthbar ultimates use the native `UnitInfoContainer`/`UnitInfoPanelSnippet`; topbar appearance stays stock. The existing one-second HUD sampler sends `HPV2_ULTIMATE_SNAPSHOT` to world contexts. Before `UltimateUnlocked`, cached `SoulsValue` text drives `clamp((souls - 600) / 3200, 0, 1) * 360`; abbreviated display values have display precision. After unlock, use native `UltimateStatusBG` clip, or 360 when ready. Missing/stale data clears the fill, not the base. Local/dead/disconnected and ambiguous names remain excluded; duplicate target dummies still lack a reliable per-player mapping. Keep native placement instead of negative-offset sibling panels.
- Profile counters `ultimatePublishedPlayers`, `ultimateReceived`, `ultimateRejected`, `ultimateUnmatched`, `ultimatePanelMissing`, and `ultimateShown` separate transport, identity, and rendering failures. Panorama rejects class suffixes after `:not(...)`; the removed topbar stylesheet caused a live startup failure despite compiling successfully.
- Render radial progress on mod-owned `HPV2UltimateProgress` inside native `unit_info_bg`, using the native ready artwork/colors. Keep `unit_ult_ready_icon` for engine binding but mask its opacity only while the host class is active. Writing native ready visibility caused live visible/collapse flicker; never compete with it. The extra progress image is the ownership boundary, not a second accessory container.
