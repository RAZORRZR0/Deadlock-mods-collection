...
$noMissingRoot = Join-Path $root 'showrank_barebones_no_missing'
$compositionScript = Join-Path $root 'scripts\profile-stats-community-composition.js'
$stagingRoot = Join-Path $root '_showrank_barebones_no_missing_build'
$noMissingStageSource = Join-Path $stagingRoot 'src'
$noMissingStageCompiled = Join-Path $stagingRoot 'src_compiled'
$vpkOutput = Join-Path $root 'showrank_barebones_no_missing_dir.vpk'
...
function Assert-NoMissingBarebonesAssetSet {
    param(
    ...
    foreach ($asset in $ExpectedAssets) {
        if (-not $expected.Add($asset)) { throw "NoMissingBarebones required asset is duplicated: $asset" }
    }
...
function Get-NoMissingBarebonesAssetPaths {
    ...
    if (-not (Test-Path -LiteralPath $panoramaRoot)) {
        throw "NoMissingBarebones panorama source folder not found: $panoramaRoot"
    }
    return @(
...
function Get-NoMissingBarebonesPackedAssetPaths {
...
function Assert-DeadlockClosed {
    ...
    if ($running.Count -gt 0) {
        throw 'Deadlock must be closed before installing ShowRank no-missing-barebones.'
    }
}

function Get-NoMissingBarebonesSha256 {
...
function Invoke-NoMissingBarebonesClosureMinification {
    param(
...
function Remove-NoMissingBarebonesInstallTemporary {
    param(
    ...
    if ((Split-Path -Leaf $Path) -ne 'pak89_dir.showrank-barebones-no-missing.tmp.vpk') {
        throw "Refusing to remove unexpected temporary install file: $Path"
    }
    Remove-Item -LiteralPath $Path -Force
}

function Install-NoMissingBarebonesVpk {
    param([Parameter(Mandatory = $true)][string]$SourceVpk)

    $destination = Join-Path $AddonsPath 'pak89_dir.vpk'
    $temporary = Join-Path $AddonsPath 'pak89_dir.showrank-barebones-no-missing.tmp.vpk'
    $replaceBackup = Join-Path $AddonsPath 'pak89_dir.showrank-barebones-no-missing.replace-backup.tmp.vpk'
    ...
    try {
        ...
        if (Test-Path -LiteralPath $replaceBackup) {
            throw "A preserved no-missing-barebones replacement backup requires recovery or removal: $replaceBackup"
        }

        Copy-Item -LiteralPath $SourceVpk -Destination $temporary -Force

        $sourceHash = Get-NoMissingBarebonesSha256 -Path $SourceVpk
        $temporaryHash = Get-NoMissingBarebonesSha256 -Path $temporary
        if (-not $sourceHash.Equals($temporaryHash, [System.StringComparison]::OrdinalIgnoreCase)) {
            throw "Installed no-missing-barebones VPK hash does not match build artifact: $temporary"
        }

        $temporaryTree = Get-PackedVpkTree -VpkEditCli $vpkEditCli -VpkPath $temporary -Source2ViewerPath $source2Viewer
        Assert-NoMissingBarebonesAssetSet -Actual (Get-NoMissingBarebonesPackedAssetPaths -Tree $temporaryTree) -ExpectedAssets $requiredCompiledAssets -Label 'Temporary no-missing-barebones VPK'
        ...
        Remove-NoMissingBarebonesInstallTemporary -Path $temporary -AddonsRoot $AddonsPath
        throw
    }
}

if (-not (Test-Path -LiteralPath $noMissingRoot)) {
    throw "ShowRank no-missing-barebones source folder not found: $noMissingRoot"
}
...
& npm --prefix $noMissingRoot run validate
if ($LASTEXITCODE -ne 0) { throw "ShowRank no-missing-barebones validation failed with exit code $LASTEXITCODE" }

Assert-NoMissingBarebonesAssetSet -Actual (Get-NoMissingBarebonesAssetPaths -RootPath $noMissingRoot) -ExpectedAssets $requiredSourceAssets -Label 'NoMissingBarebones source package'

Remove-TreeUnderRoot -Path $stagingRoot -RootPath $root -ExpectedLeaf '_showrank_barebones_no_missing_build'
if (Test-Path -LiteralPath $vpkOutput) {
    Assert-PathUnderRoot -Path $vpkOutput -RootPath $root
    if ((Split-Path -Leaf (Resolve-Path -LiteralPath $vpkOutput).Path) -ne 'showrank_barebones_no_missing_dir.vpk') {
        throw "Refusing to remove unexpected no-missing-barebones VPK: $vpkOutput"
    }
    Remove-Item -LiteralPath $vpkOutput -Force
}

try {
    New-Item -ItemType Directory -Path $noMissingStageSource -Force | Out-Null
    foreach ($asset in $requiredSourceAssets) {
        $sourcePath = Join-Path $noMissingRoot ($asset -replace '/', '\\')
        $stagedPath = Join-Path $noMissingStageSource ($asset -replace '/', '\\')
    ...
    & node $compositionScript --host-root $noMissingRoot $noMissingStageSource
    if ($LASTEXITCODE -ne 0) { throw "Profile Stats Community composition failed with exit code $LASTEXITCODE" }
    Assert-NoMissingBarebonesAssetSet -Actual (Get-NoMissingBarebonesAssetPaths -RootPath $noMissingStageSource) -ExpectedAssets $requiredSourceAssets -Label 'Composed no-missing-barebones source'
    $readableRuntime = Join-Path $stagingRoot 'showrank_barebones.readable.js'
    $stagedRuntime = Join-Path $noMissingStageSource 'panorama\scripts\showrank_barebones.js'
    Copy-Item -LiteralPath $stagedRuntime -Destination $readableRuntime -Force
    if ((Get-NoMissingBarebonesSha256 -Path $readableRuntime) -ne (Get-NoMissingBarebonesSha256 -Path $stagedRuntime)) {
        throw 'Staged no-missing-barebones runtime does not exactly match the composed readable source before minification.'
    }
    Invoke-NoMissingBarebonesClosureMinification -ReadableSourcePath $readableRuntime -StagedSourcePath $stagedRuntime -TemporaryRoot (Join-Path $stagingRoot 'minify')
    ...
    try {
        ...
        & node (Join-Path $noMissingRoot 'tests\showrank-barebones-runtime.test.js')
        if ($LASTEXITCODE -ne 0) { throw "Minified no-missing-barebones runtime test failed with exit code $LASTEXITCODE" }
        & node (Join-Path $noMissingRoot 'tests\profile-stats-community-runtime.test.js')
    ...
    $requiredCompiledOutputs = @($requiredCompiledAssets | ForEach-Object { Join-Path $noMissingStageCompiled ($_ -replace '/', '\\') })
    Invoke-Source2Compiler -CompilerPath $compiler -SourceDir $noMissingStageSource -RequiredOutputs $requiredCompiledOutputs -HiddenWindow
    Assert-NoMissingBarebonesAssetSet -Actual (Get-NoMissingBarebonesAssetPaths -RootPath $noMissingStageCompiled) -ExpectedAssets $requiredCompiledAssets -Label 'Compiled no-missing-barebones output'

    Invoke-VpkPack -VpkEditCli $vpkEditCli -InputDir $noMissingStageCompiled -OutputPath $vpkOutput
    $packedTree = Get-PackedVpkTree -VpkEditCli $vpkEditCli -VpkPath $vpkOutput -Source2ViewerPath $source2Viewer
    Assert-NoMissingBarebonesAssetSet -Actual (Get-NoMissingBarebonesPackedAssetPaths -Tree $packedTree) -ExpectedAssets $requiredCompiledAssets -Label 'Packed no-missing-barebones VPK'

    if ($Install) {
        Install-NoMissingBarebonesVpk -SourceVpk $vpkOutput
    }
} finally {
    if (-not $KeepStaging) {
        Remove-TreeUnderRoot -Path $stagingRoot -RootPath $root -ExpectedLeaf '_showrank_barebones_no_missing_build'
